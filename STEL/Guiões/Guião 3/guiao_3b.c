...
//gcc -lm guiao_3b.c listas.o -o run 
int main(int argc, char **argv){
    ...
    float Ax = atof(argv[4]);
    printf("Delay a calcular %.2f\n", Ax);

    //declare variables
    double c, d, delay[n_samples], num_c, num_d, delta_time_c, delta_time_d, dm;
    int i, j, size_histo, tipo_ev, busy, Q, count_arrivals, count_delays, total_delays, count_Ax;
    ...
    while(count_arrivals < n_samples){
        ...
        //check if is departure
            ...
            if (Q > 0 ){
                ...
                //update counts
                count_delays++;
                if(delay[count_delays-1] > Ax) count_Ax++;
    ...
    printf("Delay higher than %.2f: %.2f\n", Ax, (float)count_Ax/total_delays*100);
